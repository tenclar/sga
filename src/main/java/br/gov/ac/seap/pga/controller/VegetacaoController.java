...
import br.gov.ac.seap.pga.service.VegetacaoService;
import br.gov.ac.seap.pga.util.FacesUtils;

@Controller
@ManagedBean
@Scope("view")
public class VegetacaoController extends BaseController {
	...
	@Autowired
	private VegetacaoService vegetacaoService;

	private Vegetacao vegetacao;

	private List<Vegetacao> list = null;
	...
	public void actsalvar() {
		...
		try {
			...
			this.vegetacaoService.save(vegetacao);
	...
	public List<SelectItem> getSelectItems() {
		...
		for (Vegetacao p : this.vegetacaoService.findAll()) {
	...
	public void actnovo() {
		this.vegetacao = new Vegetacao();
		
		this.vegetacao.setUser(super.getUserLogin());
	...
	public void acteditar() {
		vegetacao = this.vegetacaoService.findById(vegetacao.getId());
		this.setActionstate(EnumActionState.FORM);
	}

	public void actlista() {
		list = this.vegetacaoService.findAll();
	...
	public void actpesquisa() {
		...
		try {
			...
			if ("nome".equals(this.tipopesquisa)) {
				this.list = this.vegetacaoService.findListByNomeLike(argumento);
			}
			if ("description".equals(this.tipopesquisa)) {
				this.list = this.vegetacaoService.findListByDescriptionLike(argumento);
			}
